...
import { Clock, LogOut, XCircle, RefreshCw, Building2 } from 'lucide-react';
import { useAuth } from '@/auth/AuthContext.jsx';
import { useNavigate } from 'react-router-dom';

export default function ProviderPendingPage({ profile }) {
    const { signOut } = useAuth();
    const navigate = useNavigate();

    // The user's status might be 'pending' or 'Rejected' (or 'suspended')
    const status = profile?.status?.toLowerCase() || 'pending';
    const isRejected = status === 'rejected' || status === 'suspended';
    const rejectReason = profile?.metadata?.rejectionReason || "Please contact support for more information.";
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-teal-50 via-white to-emerald-50 flex items-center justify-center p-6">
            ...
            <motion.div
                ...
                className="relative z-10 w-full max-w-lg bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl shadow-teal-100/60 border border-white p-10 text-center"
            >
                {/* Logo mark */}
                ...
                <button
                    ...
                    className="flex items-center gap-2 mx-auto text-sm text-slate-400 hover:text-red-500 transition-colors cursor-pointer"
                >
                    <LogOut size={15} />
                    Sign out
                </button>
